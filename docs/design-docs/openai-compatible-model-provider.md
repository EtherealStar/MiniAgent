...
MiniAgent 需要通过同一套内部接口调用不同的 OpenAI-compatible 模型供应商。更换供应商时，用户只需改变模型名、API 根地址和 API Key；`AgentLoop` 不感知具体供应商的 HTTP、SSE 或认证细节。Provider Adapter 长期持有连接配置，具体 model_id 在 AgentRun 开始时作为运行参数冻结。
...
- 只从 `.env` 加载单个 Provider Connection Configuration 和全局 model_id；
- 缺少配置时的可启动状态和调用失败语义；
...
```text
.env ----> ProviderConfigLoader ----> Provider Connection Configuration ----> ModelAdapter
  |                                                                            |
  +----> RuntimeConfig(model_id) ----> AgentRunEnvironment --------------------+
  ^                                                                            |
  +-------------------------- SetModel 原子回写                                 v
                                                                  OpenAI-compatible Provider
                                                                    /                  \
                                                            ModelEvent 流          Model list
```

配置来源与供应商协议是两个边界。`ProviderConfigLoader` 只从 `.env` 发现配置，`ModelAdapter` 只接收显式的 Provider Connection Configuration，不读取或写入 `.env`。Textual App 通过 RuntimeConfig 原子回写 `OPENAI_MODEL`，不让 Adapter 承担文件职责。
...
`OPENAI_BASE_URL`、`OPENAI_API_KEY` 和 `OPENAI_TIMEOUT_SECONDS` 形成 Provider Connection Configuration；`OPENAI_MODEL` 进入独立 RuntimeConfig，并在 AgentRun 开始时复制到 AgentRunEnvironment。发起模型调用前四项都必须有效。`OPENAI_TIMEOUT_SECONDS` 可选，默认值为 60 秒，必须是大于零的有限数值。

系统环境变量不参与 Provider 配置。配置值在启动加载后形成内存快照；运行期间手工修改 `.env` 不自动热加载。`SetModel` 必须先原子写入 `.env`，成功后再替换进程内 RuntimeConfig。

### 4.2 未配置状态

缺少或留空任何必需项不会阻止应用启动。配置加载结果必须能区分：

- `Configured`：包含完整且已验证的 Provider Connection Configuration 和 RuntimeConfig；
...
`model_id` 来自 AgentRunEnvironment，在同一 AgentRun 内保持不变。`GenerationOptions` 首版只允许可选的 `temperature`、`max_tokens` 和 `tool_choice`。调用前应验证这些值；未知选项是调用契约错误，不发送请求。
...
```json
{
  "model": "<AgentRunEnvironment.model_id>",
  "messages": [],
  "stream": true,
...
`list_models()` 调用同一 API 根地址下的 OpenAI-compatible `GET /v1/models`，将结果规范化为最小 `ModelInfo`，至少包含选择所需的 model ID 和可选显示名。它不修改当前 model_id。Coordinator 可以缓存最近一次成功结果；列表请求失败不改变当前配置，也不阻止用户写入一个非空 model_id。
...
缺少 Provider Connection Configuration、缺少 model_id 和无效调用参数发生在请求前，属于显式异常，不伪装成供应商流错误。内部不变量破坏和代码缺陷也直接抛出。
...
1. 配置只从测试提供的 `.env` 读取，缺少必需项返回 `NotConfigured` 且应用仍能启动。
...
11. `list_models()` 规范化 `/v1/models` 响应，失败不修改当前 model_id。
12. `stream()` 使用调用方传入的 AgentRun model_id，而不是 Adapter 构造期模型字段。
...
```text
miniagent/provider/config.py       Provider Connection Configuration、RuntimeConfig 和 .env 读写
