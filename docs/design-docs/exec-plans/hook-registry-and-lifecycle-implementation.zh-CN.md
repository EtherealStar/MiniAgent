...
- [ ] 在 `AgentLoop` 接入四个生命周期点，并保持提交边界、结果顺序、取消和 `turn_count` 语义（已完成两个提交后通知点；PreModelCall/PreToolUse 批次仍待 ContextManager 与 Executor 扩展）。
- [ ] 在 composition root 注册默认快筛 Hook，增加无网络行为演示和全量测试。
- [ ] 完成文档、接口导出、静态语法检查与最终验收，并填写 Outcomes。
...
已完成 Hook 模型、冻结 Registry、Dispatcher、纯快筛和两个提交后通知点。新增 Hook 测试 7 项，并单独通过。既有 loop/session/integration 测试当前被工作树中另一项未完成的 SessionEngine 重构阻塞（缺少 `commit_assistant`、`close` 等接口）；`tests/persistence/test_repository.py` 还引用尚不存在的 `miniagent.repository`。`PreModelCall` 以及 AgentLoop 中真正的 `PreToolUse` 批次计划仍未实现，因为 `ContextManager.before_model_call()` 前置接口和 ToolExecutor outcome 端口尚未在当前工作树落地；本计划不把这些部分标记为完成。
...
`miniagent/context.py` 当前实现旧 `ContextBuilder`；目标 `ContextManager` 由 `docs/design-docs/exec-plans/context-management-implementation.zh-CN.md` 规划。Model Context 是一次模型请求真正使用的不可变输入投影。`PreModelCall` 位于首次组装完成后、任何 `AssistantMessageStarted`、`turn_count` 增加或 `ModelAdapter.stream()` 之前。Hook 请求压缩只表达控制决定；ContextManager 才选择压缩范围、调用压缩模型并通过 SessionEngine 提交 ContextSummary。
...
5. 最终检查语法和无网络演示：
   ...
   演示应退出码为 0，并输出已有 provider 配置摘要、工具演示和 AgentRun 结果；若增加 phase 时间线，只应出现生命周期名称和顺序，不应出现秘密或完整输入正文。
...
不要记录完整 prompt、工具 arguments、工具结果、API Key、环境变量值或用户会话正文。
    ...
    Registry/Dispatcher：7 passed；冻结、顺序、短路、非法返回、取消与通知隔离均有覆盖。
    语法检查：uv run python -m compileall miniagent tests main.py 通过。
    回归：工作树中的 SessionEngine/persistence 未完成改动导致既有 loop/session/integration 收集或运行失败，非本计划引入。
