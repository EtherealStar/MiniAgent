...
flowchart TD
    ...
    TOOLS -->|"request target authorization"| SESSION
...
- 保留待定 Permission Request、当前 AgentRun 拒绝缓存和 Session Permission Grant，并接收 UI 的 Permission Decision；
...
SessionUpdate 是当前进程面向 UI 的单一通知流，可以包含排队状态、流式草稿、完整消息、运行状态和 Permission Request，但不参与恢复。展示性 delta 可以合并或丢失；需要用户响应的 pending permission 作为 SessionEngine 的粘性内存状态保留在 snapshot 中，并至少发布一次通知，直到裁决或取消。Trace 是可丢失的诊断数据，也不参与业务恢复。
...
工具完成严格输入校验并解析一个 ToolUse 的全部目标后，Target Authorization 使用 Current Session 冻结的 Workspace Root 分类目标。内部目标自动允许；外部目标依次检查当前 AgentRun 拒绝缓存和 Session Permission Grant，仍未裁决时由 SessionEngine 保存 pending Permission Request、通过现有 SessionUpdate 至少通知 UI 一次，并等待用户拒绝、仅此次同意或当前 Session 同意。

一个 ToolUse 的全部外部目标整体裁决；批次中的请求按原 ToolUse 顺序逐个处理，全部裁决前不启动 handler。Permission Request 没有自动超时，等待不计入工具 timeout。普通取消取消 pending 状态但保留 Session grant；切换、清空和退出清除全部 permission 内存状态。
...
- 展示性 SessionUpdate 投递失败：不回滚 Journal 或改变 AgentRun；PermissionRequested 无法至少投递一次时保守拒绝调用；
- Trace 写入失败：不改变业务结果；
...
14. Workspace Root 在 SessionEngine 生命周期内固定；越界目标未取得 Permission Decision 前不得执行 handler。
15. Permission Request、Permission Decision、拒绝缓存和 Session Permission Grant 都不参与恢复；`permission_denied` ToolResult 仍是 Journal 事实。
...
12. cwd 变化不改变 Workspace Root；workspace 内目标自动允许，越界 exact/subtree 与 read/write/delete 按声明范围裁决。
13. Permission Request 在 snapshot 中可见且至少通知一次；取消、切换、清空和退出会取消等待，恢复不会重放请求或许可。
...
- 当前目标策略仍直接拒绝绝对路径和 Workspace Root 外目标；SessionEngine、SessionUpdate snapshot 与 Textual UI 尚未实现 Target Authorization 和 Permission Request 生命周期。
