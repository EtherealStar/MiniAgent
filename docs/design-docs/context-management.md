...
本文覆盖固定 system context、ModelCall 动态工具 Prompt、工作上下文投影、Token 预算、ContextSummary 检查点、压缩生命周期和与 AgentLoop、AgentRunEnvironment、ToolView、SessionEngine 的接口。
...
- `ToolView`：当前可见工具的紧凑描述、function schema 和工具 Prompt；
...
“可用工具”由本次 ToolView 的 ToolSpec 生成紧凑索引；“工具 Prompt”只加载 ToolView 中可见工具的 PromptRef。二者必须使用同一个 ToolView，避免 Prompt 宣称某工具可用而请求中没有对应 schema。工具被禁用或不可见时，它的索引、PromptRef 和 schema 都不能进入 Model Context。
...
`AgentRunEnvironment` 至少提供：模型标识、Provider、固定 SystemContext、`context_window`、`reserved_output_tokens`、`tokenizer_encoding` 和生成限制。ToolView 独立提供本次 ModelCall 可见工具的冻结视图。
...
4. 仅启用的工具同时出现在可用工具、工具 Prompt 和 function schema 中。
5. 所有历史摘要按顺序保留；新压缩不读取、不修改旧摘要。
6. ToolUse 与 ToolResult 不能被压缩边界拆开。
7. 当前用户消息不会进入摘要。
8. 完整请求达到 80% 时恰好触发一次压缩调用。
9. 压缩后低于 50% 正常继续；高于 50% 但低于 80% 带诊断继续；达到 80% 终止。
10. Provider 返回 `prompt_tokens` 和窗口信息时，后续预算使用实际值和窗口信息。
11. 压缩输出 `finish_reason=length` 不会被保存。
12. `CompressionCompleted` 未被 SessionEngine 接受时，候选摘要不会污染 Working Context。
13. 压缩不增加 `turn_count`，目标 ModelCall 仍按真实发出次数计数。

## 11. 与现有设计的协调

本文覆盖并细化总体架构中的 ContextManager 责任。`docs/design-docs/main-loop.md` 和 `docs/design-docs/overall-architecture.md` 已同步采用“全部摘要永久保留”的语义；若未来出现冲突，应以本文定义的上下文局部边界为准并显式协调。现有 `ContextBuilder` 代码按字符计数且只支持单个摘要，不能视为本设计的完成实现。
