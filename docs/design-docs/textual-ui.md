...
SessionEngine 的 Journal 提交失败不能被 UI Update 掩盖。SessionUpdate 投递失败不回滚 Session 事实；用户重新打开 Session 时，从完整 snapshot 重建 UI Projection。
...
典型更新包括 InputQueued、InputWithdrawn、InputCommitted、AssistantStarted、AssistantDelta、AssistantDiscarded、AssistantCompleted、ToolResultCompleted、RunTerminated、QueueDiscarded 和 SessionStopped。

UI 不接收 Journal sequence、event_id、恢复 cursor 或投影 revision。message_id、part_id 和 tool_use_id 可以作为投影关联键，但不向用户展示。

SessionUpdate 允许丢失。第一版不检测事件缺口；明确打开或重新打开 Session 时以完整 snapshot 为准。
...
1. Composer 有文本时清空输入；
2. Current Session 有活动 Run 时取消该 Run；
3. 无活动 Run 时首次提示再次按下退出；
4. 1.5 秒内再次按下则有序退出。
