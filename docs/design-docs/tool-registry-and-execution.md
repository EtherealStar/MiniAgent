...
11. **结果治理**：规范序列化完整 `ToolOutput`，应用 `ResultPolicy`，必要时由 ArtifactStore 外置。
...
5. Workspace Root 判断和越界 permission 只由 Target Authorization 完成；未获授权的调用不会执行 handler，handler 不从原始输入绕过受控 targets。
...
- workspace 内 read/write/delete 自动允许，越界 exact/subtree 与 read/write/delete 分别匹配授权，能力和范围不能相互扩大；
