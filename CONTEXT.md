...
**ToolTarget（工具目标）**:
由已验证 ToolInput 派生的内部资源目标，描述规范化位置、所需 Target Capability 和 Target Scope。
...
ToolUse 对文件目标要求的操作能力，固定分为 read、write 和 delete；一种能力不隐含另一种能力。
...
在 handler 执行前统一判断全部 ToolTarget 是否位于 Workspace Root 内，并为越界目标取得 Permission Decision 的执行边界。
_Avoid_: Execution Classification、PreToolUse Hook、路径校验散点

**Permission Request（许可请求）**:
一个 ToolUse 含有尚未获准的越界 ToolTarget 时，等待用户对该调用全部越界目标作出整体裁决的内存状态。
...
Current Session 内存中按规范化目标、Target Capability 和 Target Scope 保存的许可；SessionEngine 生命周期结束时失效。
